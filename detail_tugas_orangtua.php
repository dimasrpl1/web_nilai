...
$stmt->execute();
$stmt->bind_result($anak_id);
$stmt->fetch();
$stmt->close();

if ($anak_id) {
    $stmt = $conn->prepare("SELECT NILAI FROM nilai_siswa WHERE ID_TUGAS = ? AND ID_SISWA = ?");
    $stmt->bind_param("ii", $id_tugas, $anak_id);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
...
<body>
    ...
    <p><a href="daftar_tugas_orangtua.php">Kembali ke Daftar Tugas</a></p>
</body>
</html>
