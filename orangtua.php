...
<body class="bg-gradient-to-b from-[#1E0342] to-[#433D8B] min-h-screen flex flex-col items-center justify-center p-4">
    ...
    <!-- Page Title -->
    <h1 class="text-white text-2xl sm:text-4xl font-semibold mt-10 animate-fade-in text-center">
        Daftar Mapel
    </h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 bg-white p-5 sm:p-10 rounded-lg shadow-xl mt-10 transform transition duration-500 hover:scale-105 max-w-full sm:max-w-4xl">
        ...
        foreach ($subjects as $subject) {
            echo "<a href='daftar_tugas_orangtua.php?mapel=".strtolower(str_replace(' ', '', $subject))."' class='bg-white text-[#1E0342] text-center font-semibold py-3 px-4 sm:py-4 sm:px-6 rounded-lg border-2 border-[#433D8B] hover:bg-[#433D8B] hover:text-white transition-all duration-300'>
                {$subject}
            </a>";
        }
        ?>
